...
Install_MCSManager() {
  ...
  env "npm install || Red_Error "[x] Failed to npm install in ${mcsmanager_install_path}/daemon"
  ...
  env "npm install || Red_Error "[x] Failed to npm install in ${mcsmanager_install_path}/web"
